...
build process, can be deleted by running "nmake cleanall" in a Visual Studio 2008 Command Prompt
...
source code:
  ...
  2. CD to C:\dev\SLK.  Run the following command:
     ...
     nmake is a Visual Studio tool which executes the steps in "makefile".  The makefile is
     essentially a build script: it will cause all the Visual Studio projects in SLK to be built.

     When nmake completes, you should get output like this:
...
After building SLK, you may want to change and debug code.  Here's how you typically debug SLK:
  ...
  5. Set breakpoints as needed.  For example, open AssignmentListWebPart.cs (in the SlkDll project)
     ...
     and the breakpoint should break.  (If it doesn't, do a full clean build, i.e. "nmake cleanall
